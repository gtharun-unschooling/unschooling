#!/usr/bin/env python3
"""
🎯 Create Specific Steps for ALL Activities
Write personalized, specific steps for every single activity
"""
...
def create_all_specific_steps():
    """Create specific steps for all 140 activities"""
    
    print("🎯 Creating Specific Steps for ALL Activities")
    print("=" * 60)
    ...
    def get_specific_steps(activity_id, topic, age_group, materials, objective):
        ...
        if activity_id in specific_steps:
            ...
            # Create specific steps based on activity type and topic
            return create_topic_specific_steps(topic, age_group, materials)
    
    def create_topic_specific_steps(topic, age_group, materials):
        """Create specific steps based on topic and age group"""
        
        if 'infant' in age_group.lower():
            if 'tummy' in topic.lower():
                return [
                    "Place baby on their tummy on a soft, comfortable surface",
                    "Get down on their level and make eye contact",
                    "Talk to them and encourage them to lift their head",
                    "If they get fussy, it's okay to stop and try again later",
                    "Celebrate any attempts they make to lift their head",
                    "End the session when they show signs of being done"
                ]
            elif 'music' in topic.lower() or 'sound' in topic.lower():
                return [
                    "Choose a calm time when baby is alert and happy",
                    "Play gentle, soothing music or sounds",
                    "Watch their reactions to the different sounds",
                    "Sing along softly or hum to the music",
                    "If they seem overwhelmed, turn the volume down",
                    "Stop the music when they show signs of being done"
                ]
            else:
                return [
            ...
            if 'building' in topic.lower() or 'tower' in topic.lower():
                ...
                return [
                    ...
                    "Clean up the art supplies together"
                ]
            elif 'music' in topic.lower() or 'dance' in topic.lower():
                return [
                    "Clear some space for moving around",
                    "Start with simple movements like clapping or stomping",
    ...
    print("\n🎯 Creating specific steps for all activities...")
    ...
    # Create backup
    backup_name = f'essential-growth-activities-backup-before-all-specific-steps-{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
    df.to_csv(backup_name, index=False)
    print(f"\n✅ Created backup: {backup_name}")
    
    # Save updated file
    df.to_csv('essential-growth-activities-enhanced-20250927_003226.csv', index=False)
    print("✅ Updated main file with ALL specific steps")
    
    print(f"\n📊 Changes made: {changes_made} activities")
    
    print("\n" + "=" * 60)
    print("🎉 ALL Specific Steps Created!")
    
    return True

if __name__ == '__main__':
    success = create_all_specific_steps()
    
    if success:
        print("\n🚀 Next step: Verify and sync to Google Sheets")
