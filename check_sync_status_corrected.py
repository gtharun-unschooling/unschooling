#!/usr/bin/env python3
"""
🔄 Check Sync Status - Corrected
Compare Google Sheets and the main activities CSV to ensure they are in sync
"""
...
def check_sync_status_corrected():
    """Compare Google Sheets and main activities CSV to check sync status"""
    
    print("🔄 Checking Sync Status - Google Sheets vs Main Activities CSV")
    print("=" * 70)
    ...
    try:
        # Use the main activities CSV file
        main_csv = 'essential-growth-activities.csv'
        
        if not os.path.exists(main_csv):
            print(f"❌ Main CSV file not found: {main_csv}")
            return False
        
        print(f"📁 Using main CSV: {main_csv}")
        
        # Load local CSV
        print(f"\n📖 Loading local CSV data...")
        local_df = pd.read_csv(main_csv)
        ...
        csv_mtime = os.path.getmtime(main_csv)
        csv_time = datetime.fromtimestamp(csv_mtime)
        print(f"   Local CSV: {csv_time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Get Google Sheets modification time from the sheet info
        sheet_info = spreadsheet.fetch_sheet_metadata()
        sheets_time = datetime.fromisoformat(sheet_info['properties']['modifiedTime'].replace('Z', '+00:00'))
        print(f"   Google Sheets: {sheets_time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        if csv_mtime > sheets_time.timestamp():
            print(f"   📁 Local CSV is newer (by {csv_time - sheets_time.replace(tzinfo=None)})")
        elif sheets_time.timestamp() > csv_mtime:
            print(f"   📊 Google Sheets is newer (by {sheets_time.replace(tzinfo=None) - csv_time})")
        else:
            print(f"   ✅ Both files have the same modification time")
        ...
        print(f"📁 Local CSV file: {main_csv}")
