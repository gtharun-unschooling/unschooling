#!/usr/bin/env python3
"""
🔧 Improve Activities 2-11 Simple
Improve activities 2-11 in Google Sheets one by one with delays
"""
...
def improve_activities_2_to_11_simple():
    """Improve activities 2-11 in Google Sheets one by one with delays"""
    
    print("🔧 Improving Activities 2-11 - Simple One-by-One Approach")
    print("=" * 70)
    ...
    try:
        ...
        # Update Activities 2-11 one by one
        print(f"\n📝 Updating Activities 2-11 with improved content...")
        
        for activity_num in range(2, 12):  # Activities 2-11
            if activity_num in improved_activities:
                ...
                for field, new_value in improved_activities[activity_num].items():
                    ...
                    if col_index is not None:
                        # Update the cell using direct cell reference
                        cell = worksheet.cell(row_num, col_index + 1)
                ...
                # Calculate and update validation score
                ...
                if validation_col_index is not None:
                    ...
                    validation_cell = worksheet.cell(row_num, validation_col_index + 1)
                    validation_cell.value = f"{score}/100 ({grade})"
                    worksheet.update_cells([validation_cell])
                    print(f"   ✅ Updated validation score: {score}/100 ({grade})")
                
                # Wait between activities to avoid rate limits
                if activity_num < 11:
                    print(f"   ⏳ Waiting 10 seconds before next activity...")
                    time.sleep(10)
...
if __name__ == '__main__':
    if improve_activities_2_to_11_simple():
