...
const DynamicTopicPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    console.log('🔍 DynamicTopicPage: Loading data for nicheName:', nicheName, 'topicSlug:', topicSlug);
    ...
    fetch(`${config.API_BASE_URL}/api/topics`)
      ...
      .catch(err => {
        console.error('❌ Error fetching topics data, using local fallback:', err);
        console.log('🔄 Using local topics data as fallback');
        fetch('/topicsdata.json')
          .then(res => res.json())
          .then(data => {
            setTopicData(data);
            setLoading(false);
  ...
  const ActivitiesSection = ({ topicData }) => {
    ...
    const handleStartActivity = (activityTitle) => {
      ...
      // Here you can add logic to start the activity
      // For now, we'll just show an alert
      alert(`Starting activity: ${activityTitle}`);
    };
          ...
          <div className="activities-container" style={containerStyle}>
            {activity1 && (
              <div style={activityBoxStyle1} key="activity1">
                ...
                <button
                  ...
                  onClick={() => handleStartActivity(activity1.title)}
                  onMouseEnter={(e) => {
            ...
            {activity2 && (
              <div style={activityBoxStyle2} key="activity2">
                ...
                <button
                  style={startButtonStyle}
                  onClick={() => handleStartActivity(activity2.title)}
                  onMouseEnter={(e) => {
                    e.target.style.backgroundColor = '#2563eb';
                    e.target.style.transform = 'translateY(-2px)';
                    e.target.style.boxShadow = '0 4px 8px rgba(59, 130, 246, 0.4)';
