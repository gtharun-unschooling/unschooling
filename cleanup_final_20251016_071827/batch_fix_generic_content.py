#!/usr/bin/env python3
"""
🔧 Batch Fix Generic Content
Fix generic content with proper rate limiting and batch processing
"""
...
def fix_generic_content_batch():
    """Fix generic content with batch processing and rate limiting"""
    
    try:
        print(f"🔧 BATCH FIX GENERIC CONTENT:")
        print("=" * 60)
        ...
        print(f"\n🎯 FIXING COGNITIVE SKILLS ACTIVITIES (BATCH MODE):")
        print("-" * 60)
        ...
        print(f"⏳ Processing in batches of 5 with 30-second delays...")
        
        total_updates = 0
        
        # Process in batches of 5
        for i in range(0, len(activities_to_update), 5):
            batch = activities_to_update[i:i+5]
            print(f"\n📦 Processing batch {i//5 + 1} ({len(batch)} activities):")
            
            for row_num, activity_name, age_group, category, objective in batch:
                print(f"   🔧 Row {row_num}: {activity_name}")
                
                # Generate specific content
                content = generate_specific_content(activity_name, age_group, category, objective)
                
                # Update all columns for this activity
                updates_made = 0
                ...
                print(f"      ✅ Updated {updates_made} columns")
                time.sleep(1)  # 1 second between activities
            
            total_updates += len(batch)
            
            # Wait 30 seconds between batches (except for the last batch)
            if i + 5 < len(activities_to_update):
...
def main():
    """Main function to fix generic content in batches"""
    print("🔧 Batch Fix Generic Content")
    print("=" * 50)
    print("🎯 Fix generic content with proper rate limiting")
    
    success = fix_generic_content_batch()
