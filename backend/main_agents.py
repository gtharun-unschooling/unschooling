...
def load_topics_data():
    """Load enhanced topics data from restructured JSON file."""
    try:
        # Try to load the enhanced data first
        enhanced_file = "data/topicsdata_dynamic_paths.json"
        if os.path.exists(enhanced_file):
            logger.info(f"📁 Loading enhanced topics data from {enhanced_file}")
            with open(enhanced_file, "r", encoding='utf-8') as f:
                data = json.load(f)
                # Extract topics from the enhanced data structure
                if "topics" in data:
                    topics = data["topics"]
                    logger.info(f"✅ Successfully loaded {len(topics)} enhanced topics from {enhanced_file}")
                    return topics
                else:
                    logger.warning("📁 Enhanced data structure doesn't contain 'topics' key, using original data")
                    # Fallback to original data
                    with open("data/topicsdata.json", "r", encoding='utf-8') as f:
                        original_data = json.load(f)
                        logger.info(f"✅ Successfully loaded {len(original_data)} topics from data/topicsdata.json")
                        return original_data
        else:
            # Fallback to original data
            logger.info("📁 Enhanced data not found, loading original topics data")
            with open("data/topicsdata.json", "r", encoding='utf-8') as f:
                data = json.load(f)
...
class ScheduleAgent:
    ...
    def run(self, match_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Create 4-week plan structure based on plan type
        if plan_type == "fusion":
            # Holistic Fusion Plan: All weeks follow consistent 7-category format
            weekly_plan = self._create_holistic_fusion_plan(profile, matched_topics)
        else:
            # Hybrid Monthly Plan: Different structure each week
            weekly_plan = self._create_hybrid_monthly_plan(profile, matched_topics)
        ...
        # Create smart topic distribution for 4 weeks with variety and relevance
