...
class AnalysisAgent:
    ...
    def run(self, profile_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"🔍 Analyzing {child_name} (age {child_age}) with {len(interests)} interests")
        
        # LLM Prompt for deep psychological analysis
        prompt = f"""You are an expert child development psychologist and educational specialist. Perform a comprehensive psychological and developmental analysis.

CHILD PROFILE (User-Provided Information):
- Name: {child_name}
- Age: {child_age} years old
- Interests: {', '.join(interests) if interests else 'Not specified'}
- Dislikes: {', '.join(dislikes) if dislikes else 'Not specified'}
- Preferred Learning Style: {learning_style}
- Goals: {', '.join(goals) if goals else 'Not specified'}
- Plan Type: {plan_type}

TASK: Provide a deep psychological and developmental analysis in JSON format:

{{
  "cognitive_level": "beginner/intermediate/advanced",
  "attention_span_minutes": <estimated number based on age>,
  "developmental_stage": "description of developmental stage",
  "learning_preferences": {{
    "primary_style": "{learning_style}",
    "secondary_styles": ["list other compatible styles"],
    "complexity_preference": "simple/moderate/complex"
  }},
  "personality_traits": ["inferred traits based on interests and age"],
  "motivation_drivers": ["what motivates this child based on profile"],
  "recommended_approach": "teaching approach based on analysis",
  "behavioral_insights": "learning patterns and behaviors",
  "strengths": ["identified strengths from interests and goals"],
  "areas_to_develop": ["developmental areas to focus on"],
  "engagement_strategies": ["specific strategies for engagement"],
  "social_emotional_considerations": "relevant social-emotional factors",
  "interest_analysis": {{
    "primary_interests": ["main interests"],
    "interest_patterns": "patterns in interests",
    "cross_disciplinary_opportunities": ["how interests can connect"]
  }}
}}

IMPORTANT: Base your analysis on:
1. Child's age and typical developmental milestones
2. Stated interests and dislikes
3. Chosen learning style
4. Educational psychology principles
5. Age-appropriate capabilities
...
"""
        ...
        try:
            if vertex_ai_available:
                ...
                logger.info(f"✅ LLM Analysis completed: {tokens_used} tokens")
                logger.info(f"📊 Cognitive Level: {analysis_result.get('cognitive_level')}")
                logger.info(f"📊 Attention Span: {analysis_result.get('attention_span_minutes')} minutes")
            else:
                # Minimal fallback if LLM unavailable
                analysis_result = {
                    "cognitive_level": "intermediate",
                    "attention_span_minutes": 15 if child_age < 8 else 25,
                    "learning_preferences": {"primary_style": learning_style},
                    "personality_traits": ["curious", "learner"],
                    "motivation_drivers": [learning_style]
                }
        except Exception as e:
        ...
        # Create enhanced profile combining standardized input + analysis
        enhanced_profile = {
            **standardized_profile,
            **analysis_result,
            "agent_flow": "ProfileAgent → AnalysisAgent",
        ...
        result = {
            ...
            "agent_timing": {
                ...
                "llm_prompt": prompt if llm_used else None,
                "llm_response": analysis_result if llm_used else None
            }
        }
        
        logger.info(f"✅ Analysis Agent completed in {time.time() - start_time:.2f} seconds (LLM: {llm_used})")
        return result

class MatchAgent:
    """Match Agent - Selects 50% from niches + 50% from essential growth."""
    ...
    def run(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        """Match 50% from niches + 50% from essential growth using LLM + Analysis insights."""
        start_time = time.time()
        logger.info("🎯 Match Agent: Hybrid selection (50% Niches + 50% Essential Growth)")
        ...
        enhanced_profile = analysis_result.get("enhanced_profile", {})
        
        child_name = standardized_profile.get("child_name", "Child")
        child_age = standardized_profile.get("child_age", 7)
        interests = standardized_profile.get("interests", [])
        learning_style = standardized_profile.get("preferred_learning_style", "visual")
        cognitive_level = psychological_analysis.get("cognitive_level", "intermediate")
        attention_span = psychological_analysis.get("attention_span_minutes", 20)
        ...
        niche_topics = self._llm_select_niche_topics(
            ...
            interests, 
            learning_style,
            count=40  # 80 total / 2 = 40 candidates each
        )
        
        logger.info(f"🎯 LLM selected {len(niche_topics)} CANDIDATE niche topics")
        
        # Step 4: Use LLM to select 40 CANDIDATE essential growth (50%)
        ...
        # Step 5: Combine both (40 + 40 = 80 CANDIDATES)
        matched_topics = niche_topics + eg_topics
        
        logger.info(f"🎯 Total candidates: {len(matched_topics)} topics (40 niche + 40 EG)")
        logger.info(f"📅 Schedule Agent will select final 28 from these 80 candidates")
...
class ScheduleAgent:
    ...
    def run(self, match_result: Dict[str, Any]) -> Dict[str, Any]:
        """Create weekly learning plan using LLM - Theme-based selection from 80 candidates."""
        start_time = time.time()
        logger.info("📅 Schedule Agent: Theme-based selection from candidates")
        ...
        candidate_topics = match_result.get("matched_topics", [])  # 80 candidates
        ...
        # Step 1: Use LLM to select 28 topics from 80 candidates organized by theme
        final_topics_by_week = self._llm_theme_based_selection(
        ...
        result = {
            ...
            "matched_topics": final_matched_topics,  # 28 final topics (not 80 candidates)
            "candidate_topics": candidate_topics,  # 80 candidates for reference
    ...
    def _llm_theme_based_selection(self, candidate_topics: List[Dict], child_name: str, 
                                     child_age: int, interests: List[str], learning_style: str) -> Dict[str, List[Dict]]:
        """Use LLM to select 7 topics per week from 80 candidates based on weekly themes."""
...
class ReviewerAgent:
    ...
    def run(self, schedule_result: Dict[str, Any]) -> Dict[str, Any]:
        """Review and optimize the learning plan ENTIRELY with LLM."""
        start_time = time.time()
        logger.info("🔍 Reviewer Agent: Using LLM for comprehensive plan review")
        ...
        review_analysis = {
            ...
            "attention_span": profile_analysis.get("attention_span_minutes", 15),
...
async def generate_plan(request: Request):
    ...
    try:
        ...
        # Step 2: Analysis Agent (Deep psychological analysis)
        logger.info("Step 2: Analysis Agent - Deep psychological analysis")
        analysis_result = analysis_agent.run(profile_result)
        analysis_timing = analysis_result["agent_timing"]
        
        # Step 3: Match Agent (Topic Selection using analysis)
        logger.info("Step 3: Match Agent - Topic Selection")
        ...
        # Step 4: Schedule Agent (Systematic 4-Week Planning)
        logger.info("Step 4: Schedule Agent - Systematic 4-Week Planning")
